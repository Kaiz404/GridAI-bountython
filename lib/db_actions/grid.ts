...
export async function getGrids(
  ...
  return grids.map(grid => grid.toJSON());
}
...
export async function updateGridCurrentPrice(
  ...
  return updatedGrid ? updatedGrid.toJSON() : null;
}
...
export async function getAllGridSummary(): Promise<{
  ...
  grids.forEach(grid => {
    totalProfit += grid.currentValue - grid.quantityInvested || 0;
